        ...
        else
        {
            ...
            if (SetGameManager.playerSelection[player].Count >= 3)
            {
                                         ...
                                         SetGameManager.playerSelection[player][2].cardData))
                {
                    ...
                    Debug.Log("Score: " + SetGameManager.scores[player]);
                    SetGameManager.UpdatescoreUI();

                    Debug.Log("Removing Cards");
                    foreach (var card in SetGameManager.playerSelection[player])
                    {
                        SetGameManager.dealtCards.Remove(card.cardData);
                        GridScript.cardObjects.Remove(card);
